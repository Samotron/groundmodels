...
pub struct SoilParams {
    ...
    pub mi: Option<f64>,
    pub disturbance: f64,
    pub advanced_parameters: Option<Vec<String>>,
    pub factored: bool,
...
impl SoilParams {
    ...
    pub fn remove_partial_factors(&self) -> Result<SoilParams, &'static str> {
        let pf = self.factors.as_ref().ok_or("No factors stored on this instance")?;
        let mut result = self.clone();
    ...
    pub fn mb(&self) -> Result<f64, &'static str> {
        let mi = self.mi.ok_or("mi (rock mass factor) must be set for mb calculation")?;
        let gsi = self.gsi.ok_or("gsi (geological strength index) must be set for mb calculation")?;
    ...
    pub fn s(&self) -> Result<f64, &'static str> {
        let gsi = self.gsi.ok_or("gsi (geological strength index) must be set for s calculation")?;
    ...
    pub fn a(&self) -> Result<f64, &'static str> {
        let gsi = self.gsi.ok_or("gsi (geological strength index) must be set for a calculation")?;
    ...
    fn hb_to_mc_conv(&self, sig3: f64) -> Result<f64, &'static str> {
        ...
        if ucs == 0.0 {
            return Err("ucs (unconfined compressive strength) must be nonzero for Hoek-Brown conversion");
        }
    ...
    pub fn rock_e_val(&self) -> Result<f64, &'static str> {
        let ucs = self.ucs.ok_or("ucs (unconfined compressive strength) must be set for Young's Modulus calculation")?;
        let gsi = self.gsi.ok_or("gsi (geological strength index) must be set for Young's Modulus calculation")?;
        
        let rock_val = if ucs < 100.0 { 1.0 } else { ucs / 100.0 };
        
        Ok((1.0 - (self.disturbance / 2.0)) * rock_val.sqrt() * (10.0_f64.powf((gsi - 10.0) / 40.0)))
    }
    ...
    pub fn process_layer(&self, sig3: f64, soil_type: SoilType) -> SoilLayer {
        match soil_type {
            SoilType::Rock => {
                ...
                processed.typical_description = format!("{} (processed with sig3: {})", 
...
impl GroundModel {
    ...
    pub fn get_soil_params(&self, reference: &str) -> Option<&SoilParams> {
        self.soil_params.iter().find(|param| param.reference == reference)
    }
...
mod tests {
    ...
    fn test_process_layer() {
        ...
        assert!(processed.typical_description.contains("processed with sig3: 100"));
    ...
    fn test_default_implementations() {
        let ground_model = GroundModel::default();
        assert_eq!(ground_model.soil_layers.len(), 1);
        assert_eq!(ground_model.soil_params.len(), 1);
        assert_eq!(ground_model.groundwater, 0.0);
        assert_eq!(ground_model.reference, "");
        assert!(ground_model.rigid_boundary.is_none());
        
        let soil_layer = SoilLayer::default();
        assert_eq!(soil_layer.top_level, 100.0);
        assert_eq!(soil_layer.base_level, Some(0.0));
        assert_eq!(soil_layer.reference, "");
        assert_eq!(soil_layer.unit_reference, "");
        
        let soil_params = SoilParams::default();
        assert_eq!(soil_params.reference, "");
        assert_eq!(soil_params.behaviour, SoilType::Granular);
        assert_eq!(soil_params.unit_weight, 0.0);
        assert!(!soil_params.factored);
    }
}
